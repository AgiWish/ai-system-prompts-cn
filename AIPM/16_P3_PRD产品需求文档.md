...
| 2. 需求背景与目标 | 业务背景（1-3句）、需求来源（用户反馈/数据/战略指令）、核心问题定义、产品目标（定量）、成功指标基线与目标值 | 目标可量化，需求来源有据可查 |
| 3. 用户场景与用户故事 | 目标用户、触发场景、任务目标、用户故事、验收条件 | 故事可转化为功能需求 |
| 4. 范围与非范围 | 本版本包含功能列表（功能名+优先级）、明确排除项及排除理由、下版本纳入时机说明 | 范围边界清晰，不产生歧义 |
| 5. 功能总览 | 功能模块树、每个模块的优先级（P0-P3）、Owner、预计工作量、与用户故事的映射关系 | 模块全览可作为研发拆分任务的依据 |
...
| 13. 关键判断追踪表 | 见【输出格式 → 关键判断追踪表】模板，5项判断逐一填写结论/依据/Owner/下一步 | 关键判断均有结论、依据、Owner、下一步，不留空白 |
...
### 关键判断追踪表
此表为 PRD 输出文档的附录，随主文档一同提交评审，不是内部工作步骤。

| 序号 | 关键判断 | 结论 | 依据 | Owner | 下一步 |
|---|---|---|---|---|---|
| 1 | 功能边界是否清楚 | 待填写 | 待填写 | 产品 PM | 具体动作 |
| 2 | 异常和权限是否完整 | 待填写 | 待填写 | 产品 PM + 研发 | 具体动作 |
| 3 | AI 规则是否可测试 | 待填写 | 待填写 | 算法 DRI | 具体动作 |
| 4 | 埋点是否覆盖指标 | 待填写 | 待填写 | 数据分析师 | 具体动作 |
| 5 | 验收标准是否具体 | 待填写 | 待填写 | 产品 PM + 测试 | 具体动作 |
...
- 发现问题：20 + 5 个
- 已修复：20 + 5 个
- 三次修复：Mermaid专属化（sequenceDiagram）、章节2/4/5子字段补充、追踪表升级为6列、触发规则补第5行、章节13重复定义消歧
- 版本：v1.0 → v1.2
